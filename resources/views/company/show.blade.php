    ...
    <div class="card" style="height: 80vh;overflow-y: auto;">
        <div class="card-body">
            <ul class="list-group list-group-flush">
                <li class="list-group-item"><span class="h2">{{ $company->name }}</span><img src="{{ url("storage/{$company->image}") }}" alt="{{ $company->name }}" style="max-width: 60px;" class="float-right"></li>
        ...
        <div class="card-footer">
            <form action="{{ route('company.destroy', $company->id) }}" class="form" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-outline-danger" onclick="return confirm('Tem certeza que quer apagar esta compania?')"><i class="fa fa-fw fa-trash"></i>Apagar</button>
            </form>
        </div>
    </div>
@stop